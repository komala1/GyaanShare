...
# Summary
# urlpatterns: A list of URL pattern definitions.
# path function:
# route: URL pattern to match.
# view: View function or class to handle the request.
# kwargs (optional): Additional keyword arguments for the view.
# name (optional): Name for the URL pattern.
# Example pattern path('login/', views.userlogin, name='login'):
# Matches the URL http://<domain>/login/.
# Calls views.userlogin to handle the request.
# Can be referenced by name ('login') in templates and code.



# path('route/',views.funcname,name='that like http://<domain>/name/)